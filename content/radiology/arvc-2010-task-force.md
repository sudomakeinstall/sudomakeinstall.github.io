+++
title = "ARVC 2010 Task Force Criteria"
description = "ARVC 2010 Task Force Friteria (Update)"
date = 2026-06-20
weight = 8
+++

## Categories

<ol type="I">
  <li>Global or Regional RV Dysfunction and Structural Alterations (echo, CMR, or angiography)</li>
  <li>Tissue Characterization of Wall</li>
  <li>Repolarization Abnormalities</li>
  <li>Depolarization/Conduction Abnormalities</li>
  <li>Arrhythmias</li>
  <li>Family History</li>
</ol>

## Cardiac MRI (Category I)

Regional RV akinesia, dyskinesia, or dyssynchronous contraction, plus ≥1 of:

| Criterion | Major | Minor |
|-----------|-------|-------|
| RVEDV/BSA (male) | ≥ 110 mL/m² | 100–110 mL/m² |
| RVEDV/BSA (female) | ≥ 100 mL/m² | 90–100 mL/m² |
| RVEF | ≤ 40% | 40–45% |

## Diagnostic Confidence

| Confidence | Criteria |
|------------|----------|
| Definite | 2 major OR 1 major + 2 minor OR 4 minor from different categories |
| Borderline | 1 major + 1 minor OR 3 minor from different categories |
| Possible | 1 major OR 2 minor from different categories |

## Reference

Marcus FI, McKenna WJ, Sherrill D, Basso C, Bauce B, Bluemke DA, Calkins H, Corrado D, Cox MG, Daubert JP, Fontaine G, Gear K, Hauer R, Nava A, Picard MH, Protonotarios N, Saffitz JE, Sanborn DM, Steinberg JS, Tandri H, Thiene G, Towbin JA, Tsatsopoulou A, Wichter T, Zareba W. Diagnosis of arrhythmogenic right ventricular cardiomyopathy/dysplasia: proposed modification of the Task Force Criteria. Eur Heart J. 2010 Apr;31(7):806-14. doi: 10.1093/eurheartj/ehq025. Epub 2010 Feb 19. PMID: [20172912](https://pubmed.ncbi.nlm.nih.gov/20172912/).
